...
`ArtworkThumbnail`の外部画像検索は初期状態で無効とし、利用者が設定で有効にした場合だけ自動取得する。手動のアルバム・アーティスト画像検索は、送信先と送信する名前を三言語で説明してから開始する。

また、本リポジトリのMobileは同期companionで、下記iOSシステム音楽の能力表は将来の統合方針を含む。`MPMediaQuery`による統合ライブラリの実装証拠は本リポジトリにない。全体の判断は[ADR-0003](0003-stabilization-and-component-boundaries.md)を参照する。
...
| 書誌・画像 | 条件付き | MusicBrainz、Cover Art Archive、Wikidata／Wikimedia、Apple MusicのHTTPS API | 手動書誌候補は確認後に保存。画像の自動取得は既定で無効、手動検索は送信前説明を表示 | 類似一致だけで自動上書きしない |
...
| 書誌・アート検索メタデータ | 説明後の手動候補検索、または利用者が自動取得を有効にした状態でローカル画像がないアルバム／アーティストの表示 | MusicBrainz / Cover Art Archive / Wikidata・Wikimedia / Apple Music | サービス方針に従う。承認結果、同意設定、画像キャッシュのみローカル保存 | 設定で自動取得を停止、メタデータ／アート編集、キャッシュ削除 |
...
- 外部送信を伴う検索・取り込み・書き出しは利用者の明示操作で開始する。例外となる画像の自動取得は初期状態で無効にし、設定画面の説明を読んで有効にした場合だけ実行する。
