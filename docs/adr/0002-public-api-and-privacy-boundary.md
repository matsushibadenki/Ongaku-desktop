...
本ADRの明示操作限定は目標の契約であり、現行の `ArtworkThumbnail` はローカル画像がない場合に画面表示から外部画像検索を開始する。自動取得設定・送信前説明の統一は未実装で、[ROADMAP A5](../ROADMAP.md)で対応する。この差分を解消するまで、外部送信が全て手動操作に限定されるとは保証しない。
...
| 書誌・画像 | 条件付き | MusicBrainz、Cover Art Archive、WikidataのHTTPS API | 手動書誌候補は確認後に保存。画像は現行では表示時の自動検索も行うため、同意・設定の統一をA5で実施 | 類似一致だけで自動上書きしない |
...
| 書誌・アート検索メタデータ | 手動候補検索、またはローカル画像がないアルバム・アーティストの表示 | MusicBrainz / Cover Art Archive / Wikidata | サービス方針に従う。承認結果と画像キャッシュのみローカル保存 | メタデータ／アート編集、キャッシュ削除 |
...
- 外部送信を伴う検索・取り込み・書き出しは利用者の明示操作で開始する。自動画像取得は送信前の説明と設定で制御する方針へ変更するが、現行との差分はA5完了まで残る。
